...
SRING: '"' .*? '"';  / 匹配 "..." 间的任意文本
```
其中 `?` 后缀提供了对非贪婪匹配子规则 (nongreedy subrule) 的支持; 非贪婪匹配的基本含义是: "获取一些字符, 直到发现匹配后续子规则的字符为止"; 更准确的描述是, 在保证整个父规则完成匹配的前提下, 非贪婪的子规则匹配最少的字符  
现在 STRING 规则还不够完善, 因为它不允许其中出现双引号, Antlr 支持 "\\" 转义字符, 来将双引号转义
```
STRING: '"' (ESC|.)*? '"'
fragment
ESC: '\\"' | '\\\\'    // 双字符序列 \" 和 \\
```
其中, Antlr 语法本身需要对转义字符 `\` 进行转义, 因此需要 `\\` 来表示单个反斜杠字符; `(ESC|.)*?` 循环会在看到后续子规则, 即一个未转义的双引号时终止

##### 匹配注释和空白字符
当词法分析器匹配到前面定义过的哪些词法符号时, 它会将匹配到的词法符号放入词法符号流, 然后输送给语法分析器; 但是当词法分析器匹配到注释和空白字符的时候, 我们通常希望将它们丢弃, 这样语法分析器就不必处理注释和空白字符了; 例如以下丢弃空白字符的规则
```
WS: (' '|'\t'|'\r'|'\n')+ -> skip;  // 匹配一个或多个空白字符将其丢弃
```
或者
```
WS: [ \t\r\n]+ -> skip;  // 匹配一个或多个空白字符将其丢弃
```
但当换行符即使可以忽略的空白字符, 又是命令行终止符的时候, 麻烦就来了; 换行符变成了上下文相关的 (context-sensitive) 的, 在某种语法上下文中我们应当丢弃换行符, 在另一些上下文中需要将其输送给语法分析器

下表是以下基础的语法规则

| 词法符号类型 | 描述及范例 |
| :--- | :--- |
| 标点符号 | 处理运算符和标点符号最容易的方法就是直接在语法规则中引用它们;<br/> call: ID '(' exprList ')';<br/> 也可以定义对应的词法符号标签;<br/> call: ID LP exprList RP;<br/> LP: '(';<br/> RP: ')';|
| 关键字 | 关键字是保留的标识符, 可以直接引用它们, 也可以为其定义词法符号类型;<br/>  returnStat: 'return' expr ';' |
| 标识符 | 几乎每种语言中的标识符都差不多, 它们之间的差异在于第一个字符的可选值和是否允许 Unicode 字符 |
| 数字 | - |
| 字符串 | - |
| 注释 | 匹配并丢弃注释<br/> LINE_COMMENT: '//' .*? '\n' -> skip;<br/> COMMENT: '/*' .*? '*/' -> skip;|
| 空白字符| 匹配并丢弃空白字符<br/> WS: [ \t\r\n]+ -> skip; |

#### 划定词法分析器和语法分析器的界线
由于 Antlr 的词法规则可以包含递归, 从技术的角度上来看, 词法分析器变得和语法分析器一样强大; 这意味着可以在词法分析器中匹配语法规则, 或者另一种极端是把字符看作词法符号, 然后用语法分析器来分析字符流的语法结构  
划定词法分析器和语法分析器的界线位置不是语言的职责, 而是语言编写的应用程序的职责; 以下是一些经验法则的指导
- 在词法分析器中匹配并丢弃语法分析器无须知晓的东西; 例如空白和注释
- 由于词法分析器来匹配类似标识符, 关键字, 字符串和数字的常见词法符号; 语法分析器的层级更高, 应该处理进一步抽象的词法符号
- 将语法分析器无须区分的词法结构归为同一词法符号类型
- 将任何语法分析器可以以相同的方式处理的实体归为一类
